...
| `zai` | `[providers.zai]` | `ZAI_API_KEY`, `Z_AI_API_KEY` | `ZAI_BASE_URL`, `Z_AI_BASE_URL`；默认 `https://api.z.ai/api/coding/paas/v4`；通用 API `https://api.z.ai/api/paas/v4` 和 `https://open.bigmodel.cn/api/paas/v4` | `GLM-5.3` 默认；`/model` 还列出 `GLM-5.2`, `GLM-5.1` 和 `GLM-5-Turbo` | Z.AI GLM Coding Plan 路由。三条第一方 Chat 路由（api.z.ai 的两个产品与 BigModel 通用平台端点）共享同一请求方言，因此推理控制——GLM-5.2 思考开关、分档 effort、以及把 `off` 改写为 `enabled` + `reasoning_effort: "low"` 的 GLM-5.3 强制思考改写——在 BigModel 上同样生效；`/preview` 等邻接路径保持 fail-closed。`GLM-5.3` 是默认值且是一等选择器行（`model = "GLM-5.3"` 或 `ZAI_MODEL=GLM-5.3`）；显式选择 `GLM-5.2` 保留它自己的 id。在 Z.ai 发布不同的 5.3 元数据之前，限制和推理选项继承自 `GLM-5.2`；它不带价格。未为 5.3 配置的账户上，真实调用仍可能以权利代码 1311 返回 429。 |
