...
- Direct Moonshot `kimi-k3` on the exact global `https://api.moonshot.ai/v1`
  or China `https://api.moonshot.cn/v1` endpoint uses the official
  ...
  Kimi Code membership endpoint uses its structured `/search` service. One
  invocation accepts at most eight provider-requested search tool calls before
  failing over, bounding latency and per-call billing.
...
Direct Moonshot `kimi-k3` Formula search was live-account verified on the China
endpoint; the global endpoint, legacy `$web_search`, Model Studio Token Plan,
China Zhipu `search_std`, and MiMo plugin paths remain covered by
request-contract mocks only. Those remaining verification gaps are accepted
because execution failures and responses without usable citations are recorded
in the structured receipt before Codewhale visibly continues to the configured
search backend.
