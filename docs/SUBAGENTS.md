...
`api_timeout_secs`, and 30 seconds above the resolved `tool_timeout_secs`
(so 1830 seconds with the 600-second default API timeout and the
1800-second default tool timeout):
...
`api_timeout_secs` and the resolved `tool_timeout_secs` (the child records
