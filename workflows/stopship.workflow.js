...
// 2026-09-14 scout repair: the read-only catalog carries no `File` (hidden
// replay-only alias, filtered from every model-visible surface), so the scout
// now activates `grep_files` with one `tool_search` call before searching.
// The evidence turn spans three responses (activate, search, verdict) inside
// the unchanged step/token caps.
