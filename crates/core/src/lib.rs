...
/// (TUI client envelope, vision request envelope, dynamic-tool result wait,
/// sub-agent tool timeout, MCP execute timeout, and the mirrors in
/// app-server and the MCP stdio proxy) that comments keep in sync; this
/// comment anchors the family roster — there is no shared constant across
/// the crates yet.
