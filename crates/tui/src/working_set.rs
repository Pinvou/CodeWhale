...
/// and must not seed repo-path heuristics. Keep the stored message untouched;
/// this projection is used only by working-set analysis and rebuilds.
...
mod tests {
    ...
    fn forkguard_working_set_ignores_leading_system_reminder_paths() {
        ...
        fs::write(reports.join("daily.md"), "report").expect("write");

        let input = "<system-reminder>\n\
            Do not try sudo/apt/systemctl/pkexec.\n\
            </system-reminder>\n\n\
        ...
        assert!(!ws.entries.contains_key("sudo/apt/systemctl/pkexec"));
    }

    #[test]
    fn forkguard_working_set_rebuild_ignores_leading_system_reminder_paths() {
        let tmp = TempDir::new().expect("tempdir");
        let reports = tmp.path().join("reports");
        fs::create_dir_all(&reports).expect("mkdir");
        fs::write(reports.join("daily.md"), "report").expect("write");
        ...
        assert_eq!(text, input, "working-set rebuild must not mutate history");
    }
