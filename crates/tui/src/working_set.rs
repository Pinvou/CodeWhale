...
/// and must not seed working-set path heuristics.
...
mod tests {
    ...
    fn forkguard_working_set_ignores_leading_system_reminder_paths() {
        ...
        let mut rebuilt = WorkingSet::default();
        rebuilt.rebuild_from_messages(&messages, tmp.path());
        assert!(rebuilt.entries.contains_key("reports/daily.md"));
        assert!(!rebuilt.entries.contains_key("internal/policy.md"));
    }
