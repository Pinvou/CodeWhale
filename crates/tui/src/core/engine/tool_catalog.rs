...
async fn drain_interpreter_pipe<R: tokio::io::AsyncRead + Unpin + Send + 'static>(
    mut pipe: Option<R>,
) -> Vec<u8> {
    let mut buf = Vec::new();
...
pub(super) async fn execute_code_execution_tool(
    ...
    // kill_on_drop is the cancel-path backstop: when the turn is
    ...
    // kill explicitly instead of leaving the interpreter running orphaned.
    ...
    // Drain the pipes concurrently with the wait: an interpreter blocked on
    // a full pipe buffer would otherwise never exit, turning every timeout
    // into a guaranteed kill.
    let stdout_task = tokio::spawn(drain_interpreter_pipe(stdout_pipe));
    let stderr_task = tokio::spawn(drain_interpreter_pipe(stderr_pipe));

    let output = match tokio::time::timeout(code_execution_timeout(), child.wait()).await {
        Ok(status) => {
