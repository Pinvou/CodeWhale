...
fn forkguard_shell_catalog_guidance_matches_execution() {
    ...
    // The model-facing `bash` contract tool carries the same runtime guidance.
    let contract = LowercaseBashTool;
    let contract_schema = contract.input_schema();
    let contract_command = contract_schema["properties"]["command"]["description"]
        .as_str()
        .unwrap();
    assert!(contract_command.contains(dispatcher.kind().binary()));
    ...
    assert_eq!(catalog[0].description, contract.description());
    assert_eq!(
