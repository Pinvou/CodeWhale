...
pub(super) fn command_guidance(kind: &ShellKind) -> String {
    ...
    format!("Actual execution shell: `{}`. {syntax}", kind.binary())
}
...
pub(super) fn runtime_command_guidance() -> &'static str {
    ...
    GUIDANCE.get_or_init(|| {
        format!(
            "The command to execute (action=run). {}",
            command_guidance(global_dispatcher().kind())
        )
    })
}

/// Guidance for the model-facing `bash` contract tool, whose schema has no
/// `action` parameter.
pub(super) fn contract_command_guidance() -> &'static str {
    static GUIDANCE: OnceLock<String> = OnceLock::new();
    GUIDANCE.get_or_init(|| {
        format!(
...
pub(super) fn description() -> &'static str {
    ...
    DESCRIPTION.get_or_init(|| {
        format!(
         ...
         \"wait\" blocks for a background task until completion or timeout; \
         \"interact\" sends stdin to a background task; \
         \"cancel\" kills a background task. Pass wait=false for a nonblocking task snapshot. \
         Foreground mode is for bounded commands; \
