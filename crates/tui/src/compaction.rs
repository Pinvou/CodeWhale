...
// The checkpoint-recognition family lives in the `history_recognition` leaf so
// `compaction` and `runtime_handoff` can share it without a module cycle; the
// names below stay reachable on the `compaction` path.
pub(crate) use crate::history_recognition::{
    COMPACTION_CHECKPOINT_PROVENANCE, is_compaction_checkpoint_message,
    is_generated_compaction_checkpoint,
};
pub use crate::history_recognition::{COMPACTION_SUMMARY_MARKER, LEGACY_COMPACTION_SUMMARY_MARKER};
